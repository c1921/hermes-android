...
metric disappears. If a managed-device sample exceeds the limit, CI runs one
confirmation pass and recomputes the candidate medians across both passes; the
10% limit is unchanged, so persistent regressions still fail while isolated
emulator variance does not. No physical-device result is inferred from the
emulator; physical reference-device results remain an owner-review gate.
