...
- Android is now a bounded system share target. Shared text and up to five distinct `content://` documents wait for the authenticated gateway, open a draft session, append/attach through the existing audited Hermes paths, focus the composer, and are consumed only after ingestion; nothing is sent automatically and no share payload is persisted by Android.
- Diagnostics can now save a bounded support report through Android's document picker. Report generation uses a positive field allowlist, applies defense-in-depth redaction and excludes credentials, conversations, provider settings and raw configuration; Android writes only after the user selects the destination.
...
- Remaining platform integrations that do not require a new upstream contract, excluding the completed share target.
...
For the Android share target, the deferred pass must share plain text, one image, one PDF, one generic document and five mixed documents from at least two real Android providers; confirm a disconnected share waits through reconnect, creates or reuses a draft session, opens the composer, preserves an existing draft with a blank-line separator, shows attachment chips in source order and never sends. Verify duplicate, sixth, empty, oversized, revoked-grant, malformed explicit intent, `file://` and HTTP inputs fail safely; deliver a second share during a slow attachment; rotate and recreate the process before and after consumption to check one-shot behavior. Inspect phone and expanded widths with TalkBack, 130%+ text and IME open. The cloud gate does not establish chooser registration, provider grant lifetime, layout, accessibility or device process behavior.

For diagnostic export, the deferred pass must save reports before and after doctor/security-audit runs through at least two real document providers; inspect the resulting text and confirm it contains the displayed version/connection context and bounded redacted action output but no session cookie, password, transcript, draft, provider secret, raw config or private home path. Exercise picker cancellation, revoked write access, storage-full/write failure, rotation while the picker is open, 130%+ text, TalkBack and keyboard focus. The cloud gate establishes pure report allowlisting/redaction/bounds and build integration, not Android provider behavior or visual accessibility.
