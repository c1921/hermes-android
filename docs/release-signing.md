...
Each successful `main` build also publishes a commit-addressed GitHub Release and marks it as the latest release only after all verified files are attached. The README uses GitHub's stable latest-release URLs for `hermes-android-release.apk` and `hermes-android-release.aab`, so failed builds cannot replace the current downloads.
