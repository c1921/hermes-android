...
Each successful `main` build also publishes a commit-addressed GitHub Release and marks it as the latest release only after the verified APK, AAB, R8 mapping, and `hermes-android-release.sha256` manifest are attached. The README uses GitHub's stable latest-release URLs for `hermes-android-release.apk` and `hermes-android-release.aab`, so failed builds cannot replace the current downloads.
